...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Compute cross power spectra between environment probability-weighted fields.')
    ...
    parser.add_argument('--kmin-plot', type=float, default=0.0)
    parser.add_argument('--kmax-plot', type=float, default=0.0)
...
def resolve_outdir(user_outdir):
    if user_outdir:
        ...
        if pscratch:
            outdir = Path(pscratch) / 'fisher_info' / 'pypower_cross_env_pk'
        else:
            outdir = Path.cwd() / 'outputs' / 'pypower_cross_env_pk'
...
def main():
    ...
    print('---> computing reference k-grid with unmarked field ...')
    ...
    for a, b in CROSS_PAIRS:
        ...
        if sw_a <= 0.0 or sw_b <= 0.0:
            ...
            cross_nmodes[key] = nmodes_ref.copy()
        ...
        nmodes_cross = align_to_k(k_ref, result['k'], result['nmodes'])
        cross_raw[key] = pk_cross
        cross_nmodes[key] = nmodes_cross
    ...
    csv_path = outdir / f'pk_cross_env_{tag}.csv'
    fig_path = outdir / f'pk_cross_env_{tag}.png'
    meta_path = outdir / f'run_metadata_pk_cross_env_{tag}.json'
    ...
    for a, b in CROSS_PAIRS:
        key = pair_key(a, b)
        if not nmodes_added:
            csv_cols.append(cross_nmodes[key])
            csv_header_cols.append('nmodes')
            nmodes_added = True
        csv_cols.extend([cross_raw[key], cross_used[key]])
    ...
    for a, b in CROSS_PAIRS:
        ...
        if args.kmin_plot > 0.0:
            mm &= k_ref >= args.kmin_plot
        if args.kmax_plot > 0.0:
            mm &= k_ref <= args.kmax_plot
        ax.semilogx(k_ref[mm], yy[mm], lw=1.6,
                    color=PAIR_COLORS.get((a, b)),
                    label=f'{a} x {b}')

    ax.axhline(0.0, color='grey', ls='--', lw=1.0, alpha=0.85)
    ax.set_xlabel(r'$k\ [h\,\mathrm{Mpc}^{-1}]$')
    ax.set_ylabel(r'$P_{\rm cross}(k)\ [(\mathrm{Mpc}/h)^3]$')
    title = f'{args.tracer} cross spectra  ({args.zmin:.2f} < z < {args.zmax:.2f})'
    if args.subtract_shotnoise:
        title += '  [shot-noise subtracted]'
    if args.kmin_plot > 0.0 or args.kmax_plot > 0.0:
        title += f'  [k-range: {args.kmin_plot:.3f} to {args.kmax_plot:.3f}]'
    ax.set_title(title)
    ...
    metadata = {'tracer': args.tracer,
                ...
                'kmin_plot_h_mpc': args.kmin_plot,
                'kmax_plot_h_mpc': args.kmax_plot,
