...
def resolve_outdir(user_outdir):
    if user_outdir:
        ...
        if pscratch:
            outdir = Path(pscratch) / 'fisher_info' / 'pypower_contrast_pk'
        else:
            outdir = Path.cwd() / 'outputs' / 'pypower_contrast_pk'
...
def build_contrast_mark(targetid_data, w_data, astra_prob_file, void_col, knot_col,
                        unmatched_policy, normalize_mark):
    astra = read_fits_columns_case_insensitive(astra_prob_file, ['TARGETID', void_col, knot_col])
    tid_a = np.asarray(astra['TARGETID'], dtype=np.int64)
    pvoid_a = np.asarray(astra[void_col], dtype=np.float64)
    pknot_a = np.asarray(astra[knot_col], dtype=np.float64)

    order_a = np.argsort(tid_a)
    tid_sorted = tid_a[order_a]
    pvoid_sorted = pvoid_a[order_a]
    pknot_sorted = pknot_a[order_a]

    tid_unique, unique_first = np.unique(tid_sorted, return_index=True)
    pvoid_unique = pvoid_sorted[unique_first]
    pknot_unique = pknot_sorted[unique_first]
    ...
    pvoid_matched = np.array([], dtype=np.float64)
    pknot_matched = np.array([], dtype=np.float64)
    mark_raw = np.array([], dtype=np.float64)
    mark_clipped = np.array([], dtype=np.float64)
    n_mark_outside_01 = 0
    n_invalid_prob_inputs = 0

    if np.any(matched):
        pvoid_matched = pvoid_unique[idx[matched]]
        pknot_matched = pknot_unique[idx[matched]]

        pvoid_clean = np.nan_to_num(pvoid_matched, nan=0.0, posinf=0.0, neginf=0.0)
        pknot_clean = np.nan_to_num(pknot_matched, nan=0.0, posinf=0.0, neginf=0.0)
        n_invalid_prob_inputs = int(np.sum(~np.isfinite(pvoid_matched)) + np.sum(~np.isfinite(pknot_matched)))

        pvoid_clean = np.clip(pvoid_clean, 0.0, 1.0)
        pknot_clean = np.clip(pknot_clean, 0.0, 1.0)
        mark_raw = 0.5 * (1.0 + pknot_clean - pvoid_clean)
        n_mark_outside_01 = int(np.sum((mark_raw < 0.0) | (mark_raw > 1.0)))
        mark_clipped = np.clip(mark_raw, 0.0, 1.0)

        mark[matched] = mark_clipped
    ...
    info = {'n_data': int(len(tid_d)),
            ...
            'void_col': void_col,
            'knot_col': knot_col,
            'mark_formula': 'm_i = (1 + p_knot_i - p_void_i) / 2',
            'n_invalid_probability_inputs': n_invalid_prob_inputs,
            'n_mark_outside_0_1_before_clip': n_mark_outside_01,
            ...
            'stats_pvoid_matched': summarize_array(pvoid_matched),
            'stats_pknot_matched': summarize_array(pknot_matched),
            'stats_mark_raw_matched': summarize_array(mark_raw),
            'stats_mark_used_matched': summarize_array(mark_clipped)}
...
def main():
    ...
    print(f'---> void col: {args.void_col}')
    print(f'---> knot col: {args.knot_col}')
    ...
    print('---> building contrast mark from ASTRA probabilities ...')
    mark_contrast, matched, mark_info = build_contrast_mark(targetid_data=targetid_d,
                                                            w_data=w_d,
                                                            astra_prob_file=args.astra_prob_file,
                                                            void_col=args.void_col,
                                                            knot_col=args.knot_col,
    ...
    w_dm = w_d * mark_contrast
    sw_dm = float(np.sum(w_dm, dtype=np.float64))
    sw2_dm = float(np.sum(w_dm ** 2, dtype=np.float64))
    if sw_dm <= 0.0:
        raise RuntimeError('Contrast-marked total weight is <= 0. Check matching and mark settings.')

    print('---> computing contrast-marked P(k) with PyPower ...')
    ...
    csv_path = outdir / f'pk_contrast_marked_{tag}.csv'
    fig_pk_path = outdir / f'pk_contrast_marked_{tag}.png'
    fig_ratio_path = outdir / f'pk_contrast_over_unmarked_{tag}.png'
    fig_delta_path = outdir / f'pk_contrast_minus_unmarked_{tag}.png'
    meta_path = outdir / f'run_metadata_pk_contrast_marked_{tag}.json'
    ...
    csv_header_cols = ['k_h_mpc',
                       ...
                       'pk_contrast_raw',
                       'pk_contrast_used',
                       'pk_contrast_over_unmarked',
                       'pk_contrast_over_unmarked_masked',
                       'delta_pk_contrast_minus_unmarked']
    ...
    ax.loglog(k[m_mk], pk_marked_used[m_mk], lw=1.5, color=MARKED_COLOR,
              label=r'$P_{\rm contrast}(k)$')
    ax.set_xlabel(r'$k\ [h\,\mathrm{Mpc}^{-1}]$')
    ax.set_ylabel(r'$P(k)\ [(\mathrm{Mpc}/h)^3]$')
    title = f'{args.tracer} contrast mark  ({args.zmin:.2f} < z < {args.zmax:.2f})'
    ...
    ax.set_xlabel(r'$k\ [h\,\mathrm{Mpc}^{-1}]$')
    ax.set_ylabel(r'$P_{\rm contrast}(k) / P_{\rm unmarked}(k)$')
    k_range_txt = f'{args.kmin_ratio:.3f} < k'
    if args.kmax_ratio > 0.0:
        k_range_txt += f' < {args.kmax_ratio:.3f}'
    ax.set_title(f'{args.tracer} contrast ratio  [{k_range_txt}]')
    ax.grid(alpha=0.3, which='both')
    fig.tight_layout()
    ...
    ax.set_xlabel(r'$k\ [h\,\mathrm{Mpc}^{-1}]$')
    ax.set_ylabel(r'$\Delta P(k)=P_{\rm contrast}(k)-P_{\rm unmarked}(k)$')
    ax.set_title(f'{args.tracer} contrast delta  [{k_range_txt}]')
    ax.grid(alpha=0.3, which='both')
    fig.tight_layout()
    ...
    metadata = {'tracer': args.tracer,
                ...
                'shotnoise_contrast_marked': shotnoise_marked,
                'shotnoise_contrast_marked_analytic': shotnoise_marked_analytic,
                'shotnoise_contrast_marked_pypower': pk_marked_result['shotnoise'],
                ...
                'notes': ['Contrast mark uses only p_knot and p_void.',
                          'Formula: m_i = (1 + p_knot_i - p_void_i) / 2.']}
