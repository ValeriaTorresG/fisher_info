...
def _read_raw_min_compat(raw_dir, class_dir, zone, out_tag=None):
    ...
    tag = zone_tag(zone)
    tsuf = safe_tag(out_tag)
    raw_gz = os.path.join(raw_dir, f'zone_{tag}{tsuf}.fits.gz')
    raw_fits = os.path.join(raw_dir, f'zone_{tag}{tsuf}.fits')
    raw_path = raw_gz if os.path.exists(raw_gz) else raw_fits
...
def main():
    try:
        ...
        p.add_argument('--release', choices=['EDR','DR1','DR2','YUAN23','SIM'], default='EDR',
                       help='Data release: EDR (rosettes), DR1 (mask-based NGC/SGC), DR2 (full-sky NGC/SGC split), '
                            'YUAN23/SIM (HOD snapshots cXXX_phXXX/seedX/hodXXX.fits)')
        p.add_argument('--region', choices=['N','S'], default='N',
                       help='Legacy argument kept for compatibility; ignored by DR1/DR2/YUAN23.')
        p.add_argument('--zones', nargs='+', type=str, default=None,
                       help='For DR1/DR2: zone labels (e.g., NGC SGC). '
                            'For YUAN23: snapshot labels (e.g., c000_ph000_seed0_hod000). '
                            'For EDR, ignored if --zone is given.')
        ...
        if not getattr(release_config, 'skip_preload', False):
            need_preload = True
