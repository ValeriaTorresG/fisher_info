...
VOID_COL_DEFAULT = 'PVOID'
SHEET_COL_DEFAULT = 'PSHEET'
FILAMENT_COL_DEFAULT = 'PFILAMENT'
KNOT_COL_DEFAULT = 'PKNOT'
NGC_RA_MIN_DEG = 90.0
NGC_RA_MAX_DEG = 300.0

CROSS_PAIRS = [('void', 'sheet'),
               ('sheet', 'filament'),
               ('filament', 'knot'),
               ('void', 'knot'),
               ('void', 'filament'),]

PAIR_COLORS = {('void', 'sheet'): 'deepskyblue',
               ('sheet', 'filament'): 'orange',
               ('filament', 'knot'): 'limegreen',
               ('void', 'knot'): 'magenta',
               ('void', 'filament'): 'gold',}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Compute normalized cross spectra r12 = P12 / sqrt(P1 P2) for selected environment pairs.')
    ...
    parser.add_argument('--void-col', type=str, default=VOID_COL_DEFAULT)
    parser.add_argument('--sheet-col', type=str, default=SHEET_COL_DEFAULT)
    parser.add_argument('--filament-col', type=str, default=FILAMENT_COL_DEFAULT)
    parser.add_argument('--knot-col', type=str, default=KNOT_COL_DEFAULT)
    ...
    parser.add_argument('--r-source', type=str, default='used', choices=['raw', 'used'])
    parser.add_argument('--r-denom-min', type=float, default=0.0)
    parser.add_argument('--sky-region', type=str, default='NGC', choices=['NGC', 'ALL'])
    parser.add_argument('--kmin-r', type=float, default=0.05)
    parser.add_argument('--kmax-r', type=float, default=0.2)
...
def resolve_outdir(user_outdir):
    if user_outdir:
        ...
        if pscratch:
            outdir = Path(pscratch) / 'fisher_info' / 'pypower_cross_env_r'
        else:
            outdir = Path.cwd() / 'outputs' / 'pypower_cross_env_r'
...
def compute_pk_pypower(data_positions1, data_weights1, edges, boxsize, boxcenter,
                       nmesh, resampler, interlacing,
                       random_positions1=None, random_weights1=None,
                       data_positions2=None, data_weights2=None,
                       random_positions2=None, random_weights2=None):
    kwargs = {'data_positions1': data_positions1,
    ...
    if data_positions2 is not None:
        kwargs['data_positions2'] = data_positions2
        kwargs['data_weights2'] = data_weights2

    if random_positions1 is not None:
        kwargs['randoms_positions1'] = random_positions1
        kwargs['randoms_weights1'] = random_weights1

    if random_positions2 is not None:
        kwargs['randoms_positions2'] = random_positions2
        kwargs['randoms_weights2'] = random_weights2
    elif data_positions2 is not None and random_positions1 is not None:
        kwargs['randoms_positions2'] = random_positions1
        kwargs['randoms_weights2'] = random_weights1
...
def summarize_array(values):
    ...
    if arr.size == 0:
        return {'count': 0, 'mean': np.nan, 'std': np.nan, 'min': np.nan, 'max': np.nan,
                'p16': np.nan, 'p50': np.nan, 'p84': np.nan}
...
def build_probability_marks(targetid_data, w_data, astra_prob_file, env_cols,
                            unmatched_policy, normalize_mark):
    cols_to_read = ['TARGETID'] + list(env_cols.values())
    astra = read_fits_columns_case_insensitive(astra_prob_file, cols_to_read)
    tid_a = np.asarray(astra['TARGETID'], dtype=np.int64)

    prob_cols = {}
    for env_name, col_name in env_cols.items():
        arr = np.asarray(astra[col_name], dtype=np.float64)
        arr = np.nan_to_num(arr, nan=0.0, posinf=0.0, neginf=0.0)
        arr = np.clip(arr, 0.0, 1.0)
        prob_cols[env_name] = arr

    order_a = np.argsort(tid_a)
    tid_sorted = tid_a[order_a]
    prob_sorted = {env: arr[order_a] for env, arr in prob_cols.items()}

    tid_unique, unique_first = np.unique(tid_sorted, return_index=True)
    prob_unique = {env: arr[unique_first] for env, arr in prob_sorted.items()}
    ...
    marks = {}
    mark_info = {}
    for env_name in env_cols:
        mark = np.zeros(len(tid_d), dtype=np.float64)
        prob_matched = np.array([], dtype=np.float64)
        if np.any(matched):
            prob_matched = prob_unique[env_name][idx[matched]]
            mark[matched] = prob_matched

        if unmatched_policy == 'unity':
            mark[~matched] = 1.0

        mean_before = float(np.sum(w_data * mark) / sw)
        if normalize_mark and mean_before > 0.0:
            mark /= mean_before
        mean_after = float(np.sum(w_data * mark) / sw)

        marks[env_name] = mark
        mark_info[env_name] = {'col_name': env_cols[env_name],
                               'matched_probability_stats': summarize_array(prob_matched),
                               'mark_weighted_mean_before_norm': mean_before,
                               'mark_weighted_mean_after_norm': mean_after,
    ...
    return marks, matched, info, mark_info
...
def main():
    ...
    env_cols = {'void': args.void_col,
                'sheet': args.sheet_col,
                'filament': args.filament_col,
                'knot': args.knot_col}
    ...
    marks, matched, matching_info, mark_info = build_probability_marks(targetid_data=targetid_d,
                                                                       w_data=w_d,
    ...
    for env in env_cols:
        w_env = w_d * marks[env]
        env_weights[env] = w_env
        env_weight_stats[env] = {'sum_w': float(np.sum(w_env, dtype=np.float64)),
                                 'sum_w2': float(np.sum(w_env ** 2, dtype=np.float64))}

    resampler = mas_to_resampler(args.mas)
    edges = make_k_edges(boxsize=boxsize, nmesh=args.grid)

    print('---> computing reference k-grid ...')
    pk_ref = compute_pk_pypower(data_positions1=pos_d,
                                data_weights1=w_d,
                                random_positions1=pos_r,
                                random_weights1=w_r,
                                edges=edges,
                                boxsize=boxsize,
                                boxcenter=boxcenter,
    ...
    k_ref = pk_ref['k']
    nmodes_ref = pk_ref['nmodes']
    remove_shotnoise_flag_supported = bool(pk_ref['remove_shotnoise_supported'])

    auto_raw = {}
    auto_used = {}
    auto_shotnoise = {}
    auto_shotnoise_pypower = {}
    auto_shotnoise_analytic = {}

    for env in env_order:
        sw_env = env_weight_stats[env]['sum_w']
        sw2_env = env_weight_stats[env]['sum_w2']
        print(f'---> computing auto P(k): {env}')

        if sw_env <= 0.0:
            auto_raw[env] = np.full_like(k_ref, np.nan)
            auto_used[env] = np.full_like(k_ref, np.nan)
            auto_shotnoise[env] = np.nan
            auto_shotnoise_pypower[env] = np.nan
            auto_shotnoise_analytic[env] = np.nan
            continue

        result = compute_pk_pypower(data_positions1=pos_d,
                                    data_weights1=env_weights[env],
                                    random_positions1=pos_r,
                                    random_weights1=w_r,
                                    edges=edges,
                                    boxsize=boxsize,
                                    ...
                                    resampler=resampler,
                                    interlacing=args.interlacing)
        remove_shotnoise_flag_supported &= bool(result['remove_shotnoise_supported'])

        pk_env = align_to_k(k_ref, result['k'], result['pk0'])
        auto_raw[env] = pk_env

        shot_analytic = volume * sw2_env / (sw_env * sw_env)
        shot_pypower = result['shotnoise']
        shot_use = shot_pypower if np.isfinite(shot_pypower) else shot_analytic

        auto_shotnoise_analytic[env] = shot_analytic
        auto_shotnoise_pypower[env] = shot_pypower
        auto_shotnoise[env] = shot_use
        auto_used[env] = pk_env - shot_use if args.subtract_shotnoise else pk_env.copy()

    cross_raw = {}
    cross_used = {}
    cross_shotnoise = {}
    cross_shotnoise_pypower = {}
    cross_shotnoise_analytic = {}

    for a, b in CROSS_PAIRS:
        key = pair_key(a, b)
        sw_a = env_weight_stats[a]['sum_w']
        sw_b = env_weight_stats[b]['sum_w']
        sw_ab = float(np.sum(env_weights[a] * env_weights[b], dtype=np.float64))
        print(f'---> computing cross P(k): {a} x {b}')

        if sw_a <= 0.0 or sw_b <= 0.0:
            cross_raw[key] = np.full_like(k_ref, np.nan)
            cross_used[key] = np.full_like(k_ref, np.nan)
            cross_shotnoise_pypower[key] = np.nan
            cross_shotnoise_analytic[key] = np.nan
            cross_shotnoise[key] = np.nan
            continue

        result = compute_pk_pypower(data_positions1=pos_d,
                                    data_weights1=env_weights[a],
                                    data_positions2=pos_d,
                                    data_weights2=env_weights[b],
                                    random_positions1=pos_r,
                                    random_weights1=w_r,
                                    random_positions2=pos_r,
                                    random_weights2=w_r,
                                    edges=edges,
                                    boxsize=boxsize,
                                    boxcenter=boxcenter,
        ...
        pk_cross = align_to_k(k_ref, result['k'], result['pk0'])
        cross_raw[key] = pk_cross

        shot_analytic = volume * sw_ab / (sw_a * sw_b)
        shot_pypower = result['shotnoise']
        shot_use = shot_pypower if np.isfinite(shot_pypower) else shot_analytic
        cross_shotnoise_analytic[key] = shot_analytic
        cross_shotnoise_pypower[key] = shot_pypower
        cross_shotnoise[key] = shot_use
        cross_used[key] = pk_cross - shot_use if args.subtract_shotnoise else pk_cross.copy()

    r_values = {}
    r_values_masked = {}
    r_inputs = auto_raw if args.r_source == 'raw' else auto_used
    r_cross = cross_raw if args.r_source == 'raw' else cross_used

    r_kmask = k_ref >= args.kmin_r
    if args.kmax_r > 0.0:
        r_kmask &= k_ref <= args.kmax_r

    for a, b in CROSS_PAIRS:
        key = pair_key(a, b)
        r_arr = np.full_like(k_ref, np.nan)
        r_mask = np.full_like(k_ref, np.nan)
        ...
        prod = p1 * p2
        denom = np.full_like(k_ref, np.nan)
        good_prod = np.isfinite(prod) & (prod > 0.0)
        denom[good_prod] = np.sqrt(prod[good_prod])

        good = np.isfinite(p12) & np.isfinite(denom) & (denom > args.r_denom_min)
        r_arr[good] = p12[good] / denom[good]

        good_masked = good & r_kmask
        r_mask[good_masked] = r_arr[good_masked]

        r_values[key] = r_arr
        r_values_masked[key] = r_mask

    csv_path = outdir / f'pk_cross_env_r_{tag}.csv'
    fig_path = outdir / f'pk_cross_env_r_{tag}.png'
    meta_path = outdir / f'run_metadata_pk_cross_env_r_{tag}.json'

    csv_cols = [k_ref, nmodes_ref]
    csv_header_cols = ['k_h_mpc', 'nmodes']

    for env in env_order:
        csv_cols.extend([auto_raw[env], auto_used[env]])
        csv_header_cols.extend([f'pk_auto_raw_{env}', f'pk_auto_used_{env}'])

    for a, b in CROSS_PAIRS:
        key = pair_key(a, b)
        csv_cols.extend([cross_raw[key], cross_used[key], r_values[key], r_values_masked[key]])
        csv_header_cols.extend([f'pk_cross_raw_{key}',
                                f'pk_cross_used_{key}',
                                f'r_{key}',
                                f'r_masked_{key}'])
    ...
    for a, b in CROSS_PAIRS:
        key = pair_key(a, b)
        yy = r_values_masked[key]
        mm = (k_ref > 0.0) & np.isfinite(yy)
        ax.semilogx(k_ref[mm], yy[mm], lw=1.8,
                    color=PAIR_COLORS.get((a, b)),
                    label=f'{a} x {b}')

    ax.axhline(0.0, color='grey', ls='--', lw=1.0, alpha=0.85)
    ax.axhline(1.0, color='grey', ls=':', lw=0.9, alpha=0.6)
    ax.axhline(-1.0, color='grey', ls=':', lw=0.9, alpha=0.6)
    ax.set_xlabel(r'$k\ [h\,\mathrm{Mpc}^{-1}]$')
    ax.set_ylabel(r'$r_{12}(k)=P_{12}(k)/\sqrt{P_1(k)P_2(k)}$')
    k_range_txt = f'{args.kmin_r:.3f} < k'
    if args.kmax_r > 0.0:
        k_range_txt += f' < {args.kmax_r:.3f}'
    title = (f'{args.tracer} normalized cross spectra  ({args.zmin:.2f} < z < {args.zmax:.2f})'
             f'  [{k_range_txt}]')
    if args.r_source == 'used':
        title += '  [source=used]'
    else:
        title += '  [source=raw]'
    ax.set_title(title)
    ...
    metadata = {'tracer': args.tracer,
                ...
                'astra_prob_file': args.astra_prob_file,
                'environment_probability_columns': env_cols,
                'cross_pairs': CROSS_PAIRS,
                'unmatched_policy': args.unmatched_policy,
                'mark_normalize': bool(args.mark_normalize),
                'matching_info': matching_info,
                'mark_info': mark_info,
                'environment_weight_stats': env_weight_stats,
                ...
                'r_denom_min': args.r_denom_min,
                'kmin_r_h_mpc': args.kmin_r,
                'kmax_r_h_mpc': args.kmax_r,
                'shotnoise_auto': auto_shotnoise,
                'shotnoise_auto_pypower': auto_shotnoise_pypower,
                'shotnoise_auto_analytic': auto_shotnoise_analytic,
                'shotnoise_cross': cross_shotnoise,
                'shotnoise_cross_pypower': cross_shotnoise_pypower,
                'shotnoise_cross_analytic': cross_shotnoise_analytic,
                ...
                'remove_shotnoise_flag_supported': remove_shotnoise_flag_supported}
